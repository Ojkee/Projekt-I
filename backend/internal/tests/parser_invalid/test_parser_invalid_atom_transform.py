...
CASES_PARSER_INVALID_ATOM_TRANSFORM: list[Case] = [
    Case(
        name="Slash alone",
        input="/",
        expected=[wrap_stmt(ParserErrorUserMsg.expected_expression_after("/"))],
    ),
    Case(
    ...
    Case(
        ...
        input="/= x",
        expected=[wrap_stmt(ParserErrorUserMsg.invalid_atom_prefix("="))],
    ),
    Case(
    ...
    Case(
        ...
        expected=[
            AnyNonErrorStatement(),
            wrap_stmt(ParserErrorUserMsg.expected_expression_after("/")),
        ],
    ),
]
